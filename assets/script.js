...
const MAX_PIXEL_RATIO = isLowPowerMobile ? 1 : isMobile ? 1.35 : 2;
...
// HỆ THỐNG TÁN LÁ
const particleCount = isLowPowerMobile ? 7000 : isMobile ? 12000 : 38000;
...
const photoCatalog = photoIds.map((id) => ({
  small: `./assets/photos/${id}-540.webp`,
  large: `./assets/photos/${id}-900.webp`,
}));
...
const fallingPetalsCount = isLowPowerMobile ? 28 : isMobile ? 48 : 180;
...
const starCount = isLowPowerMobile ? 160 : isMobile ? 260 : 900;
...
function createFirework(pos) {
  const pCount = isLowPowerMobile ? 28 : isMobile ? 40 : 50;
...
function loadWishImage(photo) {
  ...
  wishImage.removeAttribute("srcset");
  ...
  wishImageContainer.classList.remove("has-error");
  wishImage.sizes = "(max-width: 520px) 90vw, 340px";
  wishImage.srcset = `${photo.small} 540w, ${photo.large} 900w`;
  wishImage.src = photo.large;
}
